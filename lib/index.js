...
|> Git Chronos - Commit Engine Activated <|
    Options:
      ...
      --minCommits=<n>         Min commits per day for random range (default: 1)
      --maxCommits=<n>         Max commits per day for random range (default: 10)
      --dailyLimit=<n>         Absolute daily commit limit (default: 15)
      --commitDelayMin=<ms>    Min delay between commits (ms, default: 5000)
      --commitDelayMax=<ms>    Max delay between commits (ms, default: 55000)
...
const commitDelayMin = Number(args.commitDelayMin) || Number(process.env.COMMIT_DELAY_MIN) || 5000;
const commitDelayMax = Number(args.commitDelayMax) || Number(process.env.COMMIT_DELAY_MAX) || 55000;
...
async function runBot() {
  try {
    ...
    const plannedCommits = Math.min(getRandomCommitCount(), remainingCommits);

    await augEffects.logOperation(`CHRONOS PULSE: Preparing ${plannedCommits} commits (Daily Limit: ${CONFIG.DAILY_LIMIT})!`, CONFIG.LOG_FILE);
...
async function scheduleNextRun() {
  if (!isShuttingDown) {
    await runBot();
    const nextRun = CONFIG.ENABLE_WEEKENDS || (!isWeekend() && isWorkingHours()) ? 60 * 60 * 1000 : 5 * 60 * 1000; // 1 hour if active, 5 min if checking
