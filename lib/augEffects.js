...
const highlight = (text) => text.replace(/(Synced)/gi, `${COLORS.BLUE}$1${COLORS.RESET}`);
...
const logOperation = async (message, logFile) => {
  ...
  if (logFile) await appendFileAsync(logFile, `${message}\n`);
};
const getTime = () => new Date().toLocaleTimeString('en-US', { hour12: false });
module.exports = { format, display, logOperation, progressBar, getTime, COLORS };